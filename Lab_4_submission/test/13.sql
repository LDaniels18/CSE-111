/* How many line items are supplied by suppliers in AFRICA for orders made by customers in UNITEDSTATES? */
select count(l_orderkey) as count
...
and s_nationkey in (select s_nationkey from supplier, nation, region 
